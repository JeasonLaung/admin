		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
	            	...
	            	html += '						<input id="fileImage" type="file" size="30" name="fileselect[]" accept="image/*" '+multiple+'>';
	            	html += '					</div>';
	            	html += '				</div>';
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, response) {
						...
						if(window.FILE_RESPONSE){
							if (response && JSON.parse(response)['path']) {
								window.FILE_RESPONSE.push(JSON.parse(response)['path'])
							}
						}
						else{
